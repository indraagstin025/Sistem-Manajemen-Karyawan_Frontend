...
document.addEventListener('DOMContentLoaded', async () => {
        ...
        try {
            ...
            if (attendanceData && attendanceData.length > 0) {
                ...
                for (const attendance of attendanceData) {
                    // Gunakan getUserPhotoBlobUrl untuk mendapatkan URL foto profil karyawan
                    const photoUrl = await getUserPhotoBlobUrl(attendance.user_id, attendance.user_name, 40); // Ukuran 40x40
    ...
    const initializeAuth = async () => { // Make this async
        ...
        const adminPhotoUrl = await getUserPhotoBlobUrl(user.id, user.name, 40); // Ukuran 40x40 untuk header

        if (userAvatar) {
            userAvatar.src = adminPhotoUrl;
            userAvatar.alt = user.name || "Admin";
        }
        if (userNameNav) { // Pastikan elemen ini ada di HTML Anda

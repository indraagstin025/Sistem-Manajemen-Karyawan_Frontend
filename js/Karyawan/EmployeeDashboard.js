...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // --- Fungsi Utilitas UI (showSweetAlert, showToast, formatTime) tidak ada perubahan ---
  ...
  // --- Akhir Fungsi Utilitas UI ---


  // --- Fungsi-fungsi lain (updateAttendanceStatusUI, stopAndClearScanner, onScanSuccess, onScanFailure, loadMyTodayAttendance) tidak ada perubahan ---
  ...
  const fetchEmployeeProfileData = async () => {
    try {
      ...
      try {
        ...
        if (!user) throw new Error("Data pengguna tidak ditemukan di sesi.");
      } catch (e) {
      ...
      if (employeeData) {
        // --- [PERUBAHAN 2] --- Menggunakan photoUtils untuk mendapatkan URL foto
        const photoUrl = await getUserPhotoBlobUrl(user.id, employeeData.name);

        // Terapkan URL foto ke elemen gambar
        if (profilePhoto) {
  ...
  // --- window.openFullscreenScanner tidak ada perubahan ---
  ...
  // --- Event Listener untuk dropdown dan tombol tutup scanner tidak ada perubahan ---
  ...
  // --- Inisialisasi data utama (tidak ada perubahan) ---

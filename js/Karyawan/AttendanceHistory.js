...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const renderAttendanceCards = (data, page, limit) => {
    ...
    paginatedItems.forEach((attendance, indexInPage) => {
      ...
      const checkInDisplay = attendance.check_in && attendance.check_in.trim() !== "" ? attendance.check_in : `<span class="empty-note">-</span>`;
      ...
      } else if (attendance.status === "Tidak Absen" || attendance.status === "Alpha") {
      ...
      const finalNoteDisplay = !noteDisplayText.trim() || noteDisplayText === "-" ? `<span class="empty-note">Tidak ada catatan</span>` : `<span>${noteDisplayText}</span>`;
  ...
  const performSearch = () => {
    ...
    if (searchTerm === "") {
      ...
      filteredAttendanceData = allAttendanceData.filter((attendance) => {
        ...
        return date.includes(searchTerm) || checkIn.includes(searchTerm) || status.includes(searchTerm) || note.includes(searchTerm) || reason.includes(searchTerm);
      });
    }
